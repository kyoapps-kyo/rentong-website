<div x-data="{ open: false , atTop:true ,isMdScreen:true ,isShow:false}">
  <header id="navbar" class=" border-t-2 border-red-500 fixed top-0 z-10 bg-white w-full shadow transition-height duration-1000 ease-in-out" @scroll.window="atTop= (window.pageYOffset>100) ? false:true" @load.window="isMdScreen= (window.outerWidth < 1024) ? true:false" @resize.window="isMdScreen=(window.outerWidth < 1024) ? true:false" :class=" (atTop===true && isMdScreen===false) ||(isMdScreen===true && open===false)? 'lg:h-36 md:h-10' : 'lg:h-10 md:h-80' ">
    ...
    <div class="container mx-auto m-1 xl:px-20 md:px-4 md:relative" >
      ...
      <div class="w-full lg:w-auto lg:mt-5 lg:flex lg:justify-between lg:mb-4 md:hidden animate__animated animate__backInDown" :class=" atTop ?  '' : 'animate__backOutUp'">
        <div class="">
        ...
        <nav class=" h-16 flex items-center z-20">
          <div class=" pt-3">
            <a href="{{ route('pages.sell') }}" class=" font-bold text-xl hover:text-red-500 text-gray-500">
              {{ __('sellBuy')}}
            </a>
            <span class=" font-bold text-xl mx-6 ">/</span>
            <a href="{{ route('pages.rent') }}" class=" font-bold text-xl hover:text-green-500 text-gray-500">
              {{ __('rent')}}
            </a>
            <span class=" font-bold text-xl mx-6 ">/</span>
            <a href="{{ route('pages.projects') }}" class=" font-bold text-xl hover:text-blue-500 text-gray-500">
              {{ __('projects')}}
            </a>
            <span class=" font-bold text-xl mx-6">/</span>
            <a href="{{ route('pages.owner') }}" class=" font-bold text-xl hover:text-yellow-500 text-gray-500">
              {{ __('owner')}}
            </a>
            <span class=" font-bold text-xl mx-6">/</span>
      ...
      <nav class="lg:hidden  md:pt-10 md:flex md:flex-col md:text-right md:w-full" :class=" open ?  'md:visible animate__animated animate__backInDown' : 'md:hidden' ">
        <ul>
          <li class="  px-4 mt-2"><a href="{{ route('pages.sell') }}" class=" font-bold text-xl hover:text-red-500 text-gray-500">{{ __('sellBuy')}}</a></li>
          <li class="  px-4 mt-2"><a href="{{ route('pages.rent') }}" class=" font-bold text-xl hover:text-green-500 text-gray-500">{{ __('rent')}}</a></li>
          <li class="  px-4 mt-2"><a href="{{ route('pages.projects') }}" class=" font-bold text-xl hover:text-blue-500 text-gray-500">{{ __('projects')}}</a></li>
          <li class="  px-4 mt-2"><a href="{{ route('pages.owner') }}" class=" font-bold text-xl hover:text-yellow-500 text-gray-500">{{ __('owner')}}</a></li>
